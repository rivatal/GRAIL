...
matching x with type: intassigning x to intupdating map func for  (call printint((x: int))) : void; 
matching x with type: intupdating map func for for ((x: int) in (a: list of int)){ (call printint((x: int))) : void; 

updating map func for  (call printint((x: int))) : void; 
...
while_body:                                       ; preds = %while
  ...
  %x9 = load i32* %x
  %printf = call i32 (i8*, ...)* @printf(i8* getelementptr inbounds ([4 x i8]* @fmt2, i32 0, i32 0), i32 %x9)
  %tmp10 = load i32* %ind
  %inc = add i32 %tmp10, 1
