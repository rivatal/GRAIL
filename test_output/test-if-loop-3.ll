...
matching x with type: strmatching x with type: strassigning x to strupdating map func for  (call print((Equal: str))) : void; 
matching x with type: strupdating map func for if (((a: int) == (5: int): bool)) { (call print((Equal: str))) : void; 
; 
updating map func for  (call print((Equal: str))) : void; 
matching x with type: strupdating map func for if (((a: int) < (3: int): bool)) { (call print((Bigger: str))) : void; 
; if (((a: int) == (5: int): bool)) { (call print((Equal: str))) : void; 
; 
...
@str6 = private unnamed_addr constant [6 x i8] c"Equal\00"
...
then5:                                            ; preds = %else
  %printf6 = call i32 (i8*, ...)* @printf(i8* getelementptr inbounds ([6 x i8]* @str6, i32 0, i32 0))
  br label %merge4
