...
let rec infer_stmt (allenv: allenv) (e: stmt): (allenv * astmt) =
  (*   ignore(print_string ("\ninferring " ^ (string_of_stmt e) ^ " in " ^ (Stack.top callstack)));     *)
  ...
  let (_, genv, _, funcs) = update_map_func inferred_astmt allenv 
...
(* and update_map_funcs (astmts: astmt list) (funcs: funcs) (genv: genvironment) : funcs = 
  List.fold_left (fun a b -> update_map_func b a) funcs astmts *)
...
and annotate_expr (allenv: allenv) (e: expr) : aexpr =
  ...
  let annotated = 
    ...
    | Call(id, elist) ->    (*Function calls derive their type from the function declaration*)
      ...
      if(id=callingfunc) 
      then(
        ignore(print_string("Found " ^ id ^ " with type " ^ (string_of_type oldtype)));
        ACall(id, aelist, [], (id), oldtype)) (*first time = any, second time = type*)
      else(
        ignore(print_string("Inid !=  " ^ id ^ " = callingfunc!"));
...
and apply_update_call (call: expr) (in_id: id) : expr = 
  match call with 
  |Call(name, aelist) -> Call(in_id, aelist) 
  |x -> x

and apply_update_call_wrapper (a: stmt) (in_id: id) : stmt = 
  (*   ignore(print_string("updating map func for " ^ (string_of_stmt a))); *)
  match a with
  |Return(ae) -> Return(apply_update_call ae in_id)
  |Expr(ae) -> Expr(apply_update_call ae in_id)
  |While(ae, s1) -> 
  let as1s = List.map (fun a -> apply_update_call_wrapper a in_id) s1 in 
  While(apply_update_call ae in_id, as1s)  
  |Asn(l,ae, b) -> Asn(l, (apply_update_call ae in_id), b)
  |If(ae, s1, s2) -> let aae = apply_update_call ae in_id in 
    let as1s = List.map (fun a -> apply_update_call_wrapper a in_id) s1 in 
    let as2s = List.map (fun a -> apply_update_call_wrapper a in_id) s2 in 
    If(aae, as1s, as2s)
  |For(s1, ae, s2, s3s) -> 
    let aae = apply_update_call ae in_id in 
    let as1s = apply_update_call_wrapper s1 in_id in 
    let as2s = apply_update_call_wrapper s2 in_id in 
    let as3s = List.map (fun a -> apply_update_call_wrapper a in_id) s3s in 
    For(as1s, aae, as2s, as3s)
  |Forin(ae,ae2,s1) ->  let aae = apply_update_call ae in_id in 
    let aae2 = apply_update_call ae2 in_id in 
    let as1s = List.map (fun a -> apply_update_call_wrapper a in_id) s1 in 
    Forin(aae, aae2, as1s)

and update_map_func (a: astmt) (allenv: allenv) : allenv = 
  let env, genv, recs,funcs = allenv in
  ...
  |AExpr(ae) 
  |AWhile(ae, _) 
  |AAsn(_,ae, _,_) -> apply_update ae allenv
  |AIf(ae, s1, s2) -> 
    let allenv = apply_update ae allenv in 
    let allenv = List.fold_left (fun a b -> update_map_func b a) allenv s1 in 
    List.fold_left (fun a b -> update_map_func b a) allenv s2
  |AFor(s1, ae, s2, s3s) -> 
    let allenv = update_map_func s1 allenv in 
    let allenv = apply_update ae allenv in 
    let allenv = update_map_func s2 allenv in
    List.fold_left (fun a b -> update_map_func b a) allenv s3s
  |AForin(_,_,s1s) -> List.fold_left (fun a b -> update_map_func b a) allenv s1s
(*   |_ -> funcs *)

and apply_update (call: aexpr) (allenv: allenv) : allenv = 
  match call with 
  | AIntLit(_,_)| ABoolLit(_,_) | AFloatLit(_,_) | AStrLit(_,_) | ACharLit(_,_) | AId(_,_) -> allenv
  | AItem(_, e, _) | AUnop(_, e, _) | ADot(e,_,_) -> apply_update e allenv
  | ABinop(e1, _, e2, _) -> let allenv = apply_update e1 allenv in apply_update e2 allenv
  | AEdge(e1, _, e2, e3, _) -> let allenv = apply_update e1 allenv in 
    let allenv = apply_update e2 allenv in 
    apply_update e3 allenv
  | AList(elist, _)  -> List.fold_left (fun a b -> apply_update b a) allenv elist 
  | AGraph(elist, e1, _) -> let allenv = List.fold_left (fun a b -> apply_update b a) allenv elist in 
    apply_update e1 allenv
  | ANoexpr(_) -> allenv
  |ACall(name, aelist, astmts, id, t) -> 
    (*    ignore(print_string("updating calls for " ^ id ^ "\n"));  *)
    let allenv = List.fold_left (fun a b -> apply_update b a) allenv aelist in 
    let env, genv, recs,funcs = allenv in
    let (_, aformals, stmts) =
    ...
    ignore(print_string("updating for " ^ id ^ " in " ^ (Stack.top callstack) ^ "\n"));
    let stmts = List.map (fun a -> apply_update_call_wrapper a id) stmts in
    let env, genv, recs, funcs = allenv in 
    ignore(print_string("updating genv with " ^ id ^ " with type " ^ (string_of_type t)));
    let genv = NameMap.add id (t, aformals, stmts) genv in 
    (env, genv, recs, ((AFbody(AFdecl(id, aformals, t), astmts)) :: funcs))
  |_ -> allenv
...
and infer_func (allenv: allenv) (f: func) :  (afunc list * genvironment)  =
  ...
  match f with
  |Fbody(decl, stmts) -> 
    ignore(match decl with Fdecl(fname, _)-> 

        Stack.push fname callstack); (*set scope*)
    ...
    |Fdecl(fname, formals) ->           (*add function to NameMap*) 
      ...
      then(
        ...
        ignore(print_string("adding " ^ fname ^ " with type " ^ (string_of_type ret_type) ^ "\n"));
        let genv = NameMap.add fname (ret_type, aformals, stmts) genv in 
        let allenv = env, genv, recs, funcs in
        let (_,genv,_,funcs), astmts =  infer_stmt_list allenv stmts in 
        (ignore(Stack.pop callstack));
        ...
        let funcs = 
          ...
          |_ -> 
            if(toss) 
            then(funcs) 
            else(AFbody(AFdecl(fname, aformals, ret_type), astmts) :: funcs)
        in funcs, genv) 
